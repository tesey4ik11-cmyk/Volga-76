...
export const Header: React.FC<HeaderProps> = ({ onOpenCalcModal, onOpenAdmin }) => {
    ...
    <header className="sticky top-0 z-50 w-full bg-[#080c14]/95 backdrop-blur-md border-b border-slate-800/80">
        ...
        <div className="flex items-center gap-2">
          {onOpenAdmin && (
            <button
              type="button"
              onClick={onOpenAdmin}
              className="px-2.5 py-1.5 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 active:bg-amber-500/35 text-amber-400 text-xs font-bold border border-amber-500/30 transition-all flex items-center gap-1.5"
              title="Панель управления сметчика (Сметы, Заявки, КП)"
            >
              <span>⚙️</span>
              <span className="hidden md:inline">Сметы и КП</span>
            </button>
          )}
      ...
      {mobileMenuOpen && (
        <div className="xl:hidden border-t border-slate-800 bg-[#080c14] px-4 py-4 space-y-2">
          ...
          <div className="pt-2 flex flex-col gap-2 text-xs font-mono text-slate-300 border-t border-slate-850">
            {onOpenAdmin && (
              <button
                type="button"
                onClick={() => {
                ...
                className="w-full min-h-[44px] flex items-center justify-center gap-2 py-2.5 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-400 font-bold rounded-xl text-xs transition-colors"
              >
                <span>⚙️</span>
